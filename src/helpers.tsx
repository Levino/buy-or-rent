...
export const loanAmount = (data: Data): number => {
  const {
    buyerData: {
      size,
      brokerFee,
  ...
  return (
    size * buyPricePerSM * (1 + brokerFee + notaryFee + propertyPurchaseTax) -
    equity
  )
}
...
export const loanPaymentPerPeriod = (data: Data, period: number) => {
  const {
    loanData: { periods, interestRate },
  } = data
  let loanPayment
  if (period < periods) {
...
export const calculateEquivalentYield = memoize(
  async (data: Data) => {
    let upperLimit = 1 / 12
    let lowerLimit = 0
    let error = 1
...
const investmentPayment = (data: Data) => {
  const {
    buyerData: { buyPricePerSM, size, investmentReserve },
  } = data
...
export interface PeriodsArray {
  [key: number]: Period
}

export const calculatePeriods = (data: Data): PeriodsArray => {
  const {
    stockData: { equity, stockIncreasePerPeriod },
    taxData: { capGainsTax },
    loanData: { periods, interestRate },
    buyerData: { buyPricePerSM, size, yieldPerPeriod },
    rentData: { rentPricePerSM, rentIncreasePerPeriod },
    totalPeriods,
  } = data
  let result = {}
  let stockValue = equity
  const initialPropertyValue = size * buyPricePerSM
  ...
  for (i = 0; i <= totalPeriods; ++i) {
    ...
    const propertyValue =
      initialPropertyValue * Math.pow(1 + yieldPerPeriod, i)
    ...
    const paymentsBuyer =
      loanPaymentPerPeriod(data, i) + investmentPayment(data)
